      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-purple-700 to-pink-500 text-white px-4 py-20">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <p className="text-sm uppercase tracking-[0.5em] text-purple-200">Entrega premium</p>
            <h1 className="text-4xl md:text-5xl font-semibold leading-tight">
              Flores frescas e arranjos autorais para eternizar boas lembranças
            </h1>
            <p className="text-lg text-purple-100">
              Da vitrine à sala de estar, cada composição é feita à mão pelas floristas da Provença Flores, com flores selecionadas e embalagens que contam histórias.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link to="/catalog" className="px-6 py-3 rounded-full bg-white text-purple-600 font-semibold shadow-lg hover:bg-purple-50 transition">
                Ver catálogo
              </Link>
            ...
            {products.slice(0, 4).map((product) => (
              <div key={product.id} className="rounded-2xl overflow-hidden shadow-xl bg-white text-slate-800">
                ...
                <div className="p-4 space-y-1">
                  <p className="text-xs uppercase tracking-wide text-purple-500">{product.tagline}</p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {categories.map((category) => (
              <article key={category.id} className="rounded-2xl bg-white shadow-xl overflow-hidden flex flex-col lg:flex-row">
                <img src={category.image} alt={category.name} className="h-40 w-full lg:w-48 object-cover" />
      ...
      <section className="max-w-6xl mx-auto px-4 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="text-3xl font-bold text-slate-900">Depoimentos & carinho</h2>
          </div>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
